...
**CMA Run 5 IN PROGRESS (2026-08-27): fresh restart with physical starting materials (itu_concrete εᵣ=5.310 confirmed). scalar=+30.696 dB, 86 cal RX (0.15-0.75 km). Gen 4 eval 125 best=14.534 dB — plateau holding since gen 2 (evals 63–125, 3 gens no improvement); normal for CMA with popsize=36, covariance rotating, descent expected to resume gen 5–6 (~eval 145+). GPU memory fix (del paths + torch.cuda.empty_cache()) must be applied before eval ~590 (~13 hrs away at 87s/eval). Await FTOL → CELL 4A → CELL 8e.**
