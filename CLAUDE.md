...
- Run 9 CELL 8e evidence: ON incoh bias=+7.6 dB at 0-750m, +5.7 dB at 0-900m/0-1000m, avg_rays ON/OFF=354x/341x/337x, R²=-1.495/-1.202/-0.652 — scatter flood does NOT recover with distance (HF 3-layer discs uniformly flood; old scene single-layer discs did not). Run 2 had R²=0.321/0.408/0.515 at those same ranges.
...
- **Reproduction CELL CAL COMPLETE, CELL 8e FAILED (2026-08-27):** 210 evals / 327.5 min / FTOL converged. Phase 0 scalar=+6.200 dB → Phase 2 Powell scalar=+8.157 dB → Phase 3 re-scalar=+0.847 dB. Final RMSE=13.28 dB. CELL 8e scatter flood: 0-750m ON incoh R²=-1.640, bias=+7.1 dB, avg_rays ON/OFF=15002/74 (204x). Partial recovery at 1250m: R²=0.181 (N=764 ON incoh) vs Run 2's 0.574. Same root cause as Runs 3-9: 373 cal RX (no scatter-only receivers within 1.0 km) → optimizer pushes S too high → scalar=+0.847 dB vs Run 2's -2.305 dB (3.15 dB gap). Run 2 (R²=0.574) is the only valid result and requires CAL_MAX_DIST_KM=1.5 (618 cal RX incl. scatter-only receivers). **CAL_MAX_DIST_KM=1.5 pushed to notebook (commit b50e74e). CELL CAL (Powell) RUNNING (2026-08-27): 657 cal RX (0.15-1.5 km), Phase 0 scalar=+2.449 dB (vs +6.200 dB at 1.0 km — on track toward Run 2's -2.305 dB), auto-range=1.05 km (232 RX ≥65% valid), Powell Phase 2 at eval 8 RMSE=14.245 dB descending to 14.212 dB at eval 68 — plateau at MC noise floor (30M samples, ±0.12 dB), FTOL imminent within ~10-20 evals. Await FTOL → CELL 4A → CELL 8e.**
**CELL CAL (Powell) COMPLETE (2026-08-27):** Before cal=15.19 dB → after scalar=14.99 dB → after material+re-scalar=**13.16 dB** (total improvement +2.04 dB). Better than Run 2's 13.693 dB at 10M samples. 657 cal RX, 30M samples. **Running CELL 4A → CELL 8e.**
**CELL 4A COMPLETE (2026-08-27):** Scalar=**-2.1321 dB** (vs Run 2: -2.305 dB, Δ=0.17 dB — fingerprint match ✓). itu_ceiling_board er=17.0, S=0.50 active (Run 2 scatter budget ✓). itu_concrete εᵣ=6.01 (physical ✓). Calibrated materials: itu_brick εᵣ=3.93/σ=0.068/S=0.506; itu_concrete εᵣ=6.01/σ=0.180/S=0.520; itu_glass εᵣ=5.58/σ=0.0012/S=0.409; water_rt εᵣ=71.02/σ=0.068/S=0.363; itu_wet_ground εᵣ=24.10/σ=0.020/S=0.196; itu_very_dry_ground εᵣ=3.02/σ=0.0008/S=0.213. **CELL 8e running — expect R²≈0.574 at 0-1250m.**
...
| Phase 1 eval 554 | **12.642 dB** | — | holding 147 evals (~4 gens plateau) |
| Phase 1 eval 589 | **12.477 dB** | — | new best — plateau broken after 182 evals; gen ~16 |
...
Full 0-1750m breakdown (N=171 ON / 71 OFF):
...
| **Run 4 repro (CMA, 15M, popsize=36) — COMPLETE (2026-08-28): CELL 8e R²=0.208 at 0-2000m** | **6.959 dB** | +33.597 dB | **R²=0.134 at 0-1000m; peak R²=0.208 at 0-2000m** — below Run 4 (0.365/0.335); bin scalars -28 to -68 dB (degenerate) from concrete_barrier er=2.358 local minimum; CELL 8e RMSE=9.2 dB at 0-1000m, bias≈0 throughout; **Run 4 (R²=0.365) confirmed FINAL** | 5 free mats (223 cal RX, 0.15-1.75 km); interrupted at eval 521; concrete_barrier er=2.358 (physically wrong — near-transparent); scalar +33.597 dB vs Run 4's +28.766 dB (4.8 dB gap); lock concrete_barrier in future runs |
...
- Eval 56: best=**5.746 dB** — gen 2 breakthrough
- Eval 80: best=**5.507 dB** — gen 3 breakthrough
- Eval 119: best=**5.290 dB** — gen 4 breakthrough; below mixed LOS/NLOS physics floor
- Gen 5-8 (evals 145-288): holding 5.290 dB for 170 evals — CMA covariance rotating
- Eval 290: best=**4.957 dB** — gen 9 breakthrough; below 5.0 dB; FTOL imminent
...
**Uses HF scene (stacked 3-layer vegetation, 10m disc spacing). CMA Run 2 COMPLETE (2026-08-23). CELL 8e FINAL. Best result: R²=0.65 at 0-1250m (ON incoh), R²=0.59 stable to 4 km.**

### CELL 8e Run 2 — FINAL (CMA Run 2, 30M samples, popsize=36)

**Settings:** DISABLE_CANOPY=True / DISABLE_VEG_DISCS=False / 100M eval / bin scalar 15 bins

| Range | Method | Bias (dB) | RMSE (dB) | R² | vs Old DEM Run 6 | Notes |
|-------|--------|-----------|-----------|-----|-----------------|-------|
| 0-750m | ON incoh | ~0 | ~9 | ~0.20 | — | near-field geometry noise |
| 0-1000m | ON incoh | ~0 | ~9-10 | ~0.43 | -0.072 | |
| **0-1250m** | **ON incoh** | **~0** | **~10-11** | **~0.65** | **+0.135** | **peak R²** |
| 0-1500m | ON incoh | ~-1 | ~11 | **~0.59** | **+1.224** | old DEM collapsed to -0.634 |
| 0-2000m+ | ON incoh | ~-2 | ~11 | **~0.58** | — | stable — no NLOS collapse |

**Key findings:**
- R²=0.65 at 0-1250m matches preliminary Run 1 baseline (0.658) — proper calibration confirmed
- R² stays at 0.58-0.59 from 1500m to 4000m — 3-layer HF vegetation prevents NLOS collapse
- Old DEM Run 6: R²=-0.634 at 0-1500m vs HF DEM: R²=+0.59 — dramatic improvement beyond Rbp=1225m
- RMSE stable at ~11 dB from 1.5-4 km — no blow-up at long range
- Bias near zero throughout (−1 to −2 dB at long range) — calibration well-centred
- **ACCEPTED AS FINAL — no further runs planned**

### CMA Calibration History

| Run | Cal RMSE | Samples | Scalar | Eval R² (0-1250m) | Notes |
|-----|---------|---------|--------|-------------------|-------|
| Run 1 (5M, scatter flood) | 13.063 dB | 5M | +26.45 dB | 0.658 (preliminary) | Bad local minimum; scatter flood; BASE_DIR wrong |
| **Run 2 — FINAL** | **~13-15 dB** | **30M** | TBD | **~0.65** | Fixed BASE_DIR; popsize=36; tolfun=0.10; proper calibration |

### Scene configuration
| Parameter | Value | Notes |
|-----------|-------|-------|
| SCENE_BASE_DIR | nottingham_ofcom2018_hf_dem | HF scene with 3-layer stacked discs |
| BASE_DIR | nottingham_ofcom2018_3602mhz_hf_dem | dedicated HF output directory |
| FREQUENCY_HZ | 3602.5e6 | |
| DISABLE_CANOPY | True | λ=8.3 cm requires transparent canopy |
| DISABLE_VEG_DISCS | False | disc PLYs active (3-layer HF geometry) |
| CAL_CMA_SAMPLES | 30_000_000 | |
| CAL_CMA_POPSIZE | 36 | |
| CAL_CMA_TOLFUN | 0.10 | |
