...
### 2.1.4 Vegetation Attenuation, Terrain Effects, and Differentiable Ray Tracing

Three further considerations recur throughout the literature and are directly relevant to this thesis's scope.

**Vegetation attenuation.** Foliage introduces attenuation that is difficult to model geometrically: the empirical Weissberger model, developed from measurements between 230 MHz and 95 GHz, was among the first widely used formulations for foliage-obstructed links [18], and has since been supplemented by the more recent ITU-R Recommendation P.833-10, which extends vegetation attenuation modelling up to 100 GHz using a saturating exponential form [16]. Both remain empirical corrections layered on top of, rather than derived from, the underlying geometric propagation mechanism described in Section 2.1.2. (The dual-slope breakpoint distance that also recurs in this literature is treated separately, with its formula, in Section 2.1.8.)

**Terrain effects and elevation data quality.** Ray-tracing-based path-loss prediction depends on terrain elevation to determine line-of-sight and diffraction geometry, so the accuracy of the underlying terrain model directly bounds achievable prediction accuracy — a consideration that becomes dominant in rural, terrain-driven environments in a way it does not in dense urban settings where building geometry is the primary influence. Terrain data sources vary substantially in quality: a comprehensive comparison of freely available global digital elevation models against LiDAR-grade reference data found the 30 m SRTM product carries vertical error on the order of several metres — worse over higher-slope terrain — against sub-metre-to-few-metre accuracy for airborne LiDAR-derived digital terrain models [30]. This resolution gap is directly relevant to this thesis's own site comparison: the Nottingham and London scenes use 1 m EA LiDAR terrain data, while the Scar Hill rural site relies on 30 m SRTM data (EA LiDAR coverage does not extend north of approximately 55.9°N), providing a natural test of how terrain data resolution constrains achievable accuracy independent of the calibration methodology itself (Chapters 3–5).

**Differentiable Ray Tracing.** Recent years have also seen the emergence of GPU-accelerated, differentiable ray tracers, notably Sionna RT, built on the Mitsuba 3 rendering engine [13] and the Dr.Jit differentiable compiler [12], which make gradient-based calibration of material and scene parameters computationally practical at a scale not previously accessible [3]. This capability is what this thesis's own calibration methodology (Section 3.4) relies on.
