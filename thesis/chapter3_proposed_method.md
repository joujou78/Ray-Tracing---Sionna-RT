...
The overall workflow, applied independently at each site/frequency combination — Nottingham at 915/1802/2695/3602 MHz, London at 915/1802 MHz (2695 MHz calibration in progress at the time of writing), Scar Hill at 915 MHz, and Stevenage at 915/1802/2695/3602 MHz — follows the pipeline detailed in full in Section 3.4 (Figure 3.2). Figure 3.1 shows the geographic distribution and environment type of these four originally-selected sites. A fifth site, Southampton (915/1802 MHz, with 2695/3602 MHz notebooks in progress at the time of writing), was added later in the project and is reported alongside the others in Chapters 4–5; its transmitter coordinates (50.9464°N, −1.3101°E, from the site's own CSV header) have since been confirmed, but it is not shown in Figure 3.1, which was produced before that confirmation and has not been regenerated.

![Figure 3.1 — Ofcom 2018 measurement sites used in this thesis](figures/fig3_3_site_map.png)

*Figure 3.1 — The four Ofcom 2018 measurement sites used in this thesis, spanning dense urban (Nottingham, London), suburban/new-town (Stevenage), and rural hilltop (Scar Hill) environments. London's marker uses an approximate site coordinate, as the exact transmitter location is documented in the site-specific CSV header rather than reproduced here. Site plotted with Cartopy/Natural Earth coastline data.*
...
| **Mitsuba 3** + **Dr.Jit** | Rendering/geometry backend and differentiable JIT compiler underlying Sionna RT | Required dependency of Sionna RT; `cuda_ad_rgb` variant enables GPU execution (Section 4.2) | [12], [13] |
| **UK Environment Agency LiDAR** (1 m DTM/DSM, merged and nDSM-derived) | Terrain elevation and vegetation-canopy-height data for scene construction (Nottingham, London, Stevenage — Hertfordshire is within EA coverage, confirmed against the Stevenage-specific scene-builder notebook, Table 4.6) | Highest-resolution open terrain data available for England; nDSM (DSM − DTM) recovers above-ground clutter height not present in OSM tags | [8] |
...
| **NVIDIA Tesla V100-SXM2-16GB GPU**, 12-vCPU Xeon Platinum 8176, 62 GiB RAM VM | Compute platform for all ray-tracing runs | Confirmed via `nvidia-smi`/`lscpu`/`free -h` (Section 4.2); GPU acceleration is what makes the 30–100 million sample counts used here practical | — |
...
![Figure 3.4 — Vegetation canopy disc-layer geometry](figures/fig3_4_vegetation_discs.png)

*Figure 3.4 — Vegetation canopy representation: (a) single flat disc at canopy top, used for the 915/1802 MHz scenes; (b) three stacked discs at 30%, 65%, and 100% of crown depth below the canopy top, used for the shared 2695/3602 MHz scene. Both are surface-scattering approximations of a volumetric canopy (Section 2.1.7) — bulk attenuation is still applied post-hoc via Weissberger/ITU-R P.833-10, not by the disc geometry itself.*
...
**[FIGURE 3.3 — PLACEHOLDER — requires a real scene screenshot, not fabricated]**
*Side-by-side comparison of coarse vs. detailed urban geometry for the Nottingham scene: (a) OSM-only building footprints with default heights, (b) full LiDAR/nDSM-informed geometry with Blender-refined building detail. This environment has no access to the actual 3D scene assets or a running Sionna RT instance, so this cannot be generated here without fabricating a result. To produce it: run `sionna019_scene_builder.ipynb` CELL PREVIEW (or CELL 6c for the 2D OSM+building view) once with `INCLUDE_*` flags limited to OSM footprints only, screenshot it as panel (a); re-run with the full feature set for panel (b); combine side by side.*

**1. Scene construction.** Terrain (DTM/DSM/nDSM) and building/vegetation/infrastructure geometry are built as summarised in Figure 3.2. Two consistency rules are enforced before any run: the scene's bounding-box origin (`SCENE_WEST`, etc.) must match exactly between the scene-builder and simulation notebooks (a mismatch of 0.025° was found to cause an ~855 m coordinate offset, placing buildings incorrectly), and the terrain mesh and all building/vegetation meshes must share the same scene centre — verified with a bounding-box consistency check comparing the terrain mesh's actual half-span against the value expected from the configured scene extent.
...
where RMSE(θ) is Equation (2.15), evaluated over a calibration receiver subset restricted to the LOS-regime distance range (Section 3.3), and the minimisation is carried out with Powell's derivative-free conjugate-direction method [28] (via SciPy's optimiser) in three phases (Figure 3.5):
...
**[FIGURE 3.7 — PLACEHOLDER — requires real simulation output, not fabricated]**
*Coverage map comparison: `RadioMapSolver` 2D received-signal-strength heatmap over the Nottingham scene, overlaid with the Ofcom drive-test receiver points colour-coded by measured RSSI, for visual comparison against the simulated heatmap. This environment has no GPU, no Sionna RT installation, and no access to the calibrated scene/material files needed to run `RadioMapSolver`, so this cannot be generated here without inventing a result. To produce it: run CELL 9 (`RadioMapSolver`) in the relevant simulation notebook after the final calibration is loaded, export the coverage map figure, and overlay the receiver-level measured/simulated comparison from CELL REPORT.*

**7. Verification and quality-control steps.** Several checks were built into the pipeline specifically to catch simulation artefacts rather than treat every RMSE change as a genuine physical finding: a 3-probe sensitivity check was added after an early run showed flat RMSE across all Powell evaluations, traced to Dr.Jit kernel caching rather than an actual lack of sensitivity to the parameters being varied; a memory-leak fix was required after `PathSolver` results were found to accumulate in GPU memory across evaluations; and the fixed random seed (Section 3.3) was adopted after an unseeded run showed ±4 dB drift between nominally identical evaluations. A more involved case illustrates why these checks matter: at 2695 MHz, one calibration run (Run 3) silently made the vegetation-disc material transparent (ε_r=1) during Powell optimisation rather than during evaluation, which let the optimiser drive brick/concrete scattering coefficients up to compensate — producing a scene with roughly three times the intended scatter-path count and a fully sign-flipped bias once evaluated with the intended (non-transparent) materials; reverting the material-fixing logic so calibration and evaluation used consistent materials throughout (Run 2) resolved it (Figure 3.8). Each of these was diagnosed by comparing expected versus observed behaviour under a controlled change, consistent with standard debugging/verification practice for a computational pipeline of this complexity.

![Figure 3.8 — Verification artefact: 2695 MHz Run 2 vs. Run 3](figures/fig3_8_verification_run2_vs_run3.png)

*Figure 3.8 — Documented evidence of the material-consistency verification case: Run 3's transparent-during-calibration-only vegetation material produced roughly a third of Run 2's scatter-path count and a fully sign-flipped bias, collapsing R² from 0.574 to −0.881 at 0–1250 m. Restoring consistent materials between calibration and evaluation (Run 2) is the result reported in Chapters 4–5.*

**Reproducibility.** Every step above is fixed by an explicit, version-controlled artefact rather than an unrecorded manual action: the pinned software environment (Section 4.2), the scene-construction and simulation notebooks themselves (frozen once validated, Section 3.3), the ITU-R-sourced initial material values (Table 3.2), the fixed random seed, and the numbered equations defining every metric and optimisation objective (Equations 2.4–2.16, 3.1) used to accept or reject a given configuration. Taken together, this specification is intended to be sufficient for an independent researcher to reconstruct each site's scene, run the same three-phase calibration, and obtain the same evaluation statistics reported in Chapters 4–5, modulo the residual Monte Carlo noise floor quantified in Section 3.3.
