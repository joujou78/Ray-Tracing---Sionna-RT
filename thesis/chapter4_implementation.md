...
**[FIGURE 4.3 — PLACEHOLDER — requires real notebook output, not fabricated]**
...
*nDSM clutter-height heatmap (`CELL 3c`): the LiDAR-derived above-ground height raster (DSM − DTM) used to inform building heights and vegetation placement, for the Nottingham scene.*
...
**LiDAR terrain and vegetation data.** UK Environment Agency 1 m DTM and DSM tiles [8] are queried from the EA's public WCS endpoint for the scene's bounding box (with a padding margin, `TERRAIN_PAD_M = 3000`, so that terrain features just outside the simulated area still contribute correctly to diffraction geometry at the scene edge), downloaded as GeoTIFF tiles, and merged into single rasters. The normalised surface model — nDSM = DSM − DTM — is then computed as simple raster subtraction, giving above-ground height directly, which is what drives both building-height completion (where OSM height tags are missing or implausible) and vegetation canopy placement (Section 2.1.4). For Scar Hill, outside EA coverage, the equivalent input is 30 m SRTM data obtained via a different provider (auto-selected based on scene latitude), which is why this site's terrain resolution differs categorically from the English sites (Section 3.3).

**OpenStreetMap vector data.** Building footprints, roads, waterways, and land-use polygons are queried via `osmnx`/`geopandas` for the same bounding box, tag-filtered to the feature classes relevant to the scene (buildings, highways, waterways, natural/landuse polygons for vegetation), and converted to local scene coordinates. Building height is taken from OSM's `height`/`building:levels` tags where present and plausible, and backfilled from the nDSM raster where absent — consistent with the "importance of geometry fidelity" discussion in Section 2.1.3.

All three sources are combined only after independently being reprojected into the same local scene coordinate frame (WGS84 → EPSG:27700 → scene-local metres), which is itself a processing step worth stating explicitly: a reprojection error at this stage would silently misalign every subsequent geometry and measurement comparison, which is precisely the class of error the bounding-box and terrain consistency checks in Section 3.4 and Section 4.4 exist to catch.
...
Note: the coverage-map comparison and coarse-vs-detailed geometry figures already flagged as placeholders in Chapter 3 (Figures 3.3 and 3.7) are not repeated here — they belong to the methodology-level illustration of those concepts, not to this chapter's implementation-level record.

---

*References for this chapter reuse [3], [5], [8], [17], [28] from Chapters 1–3. See `references.md` for the full, verified reference list shared across all chapters.*
