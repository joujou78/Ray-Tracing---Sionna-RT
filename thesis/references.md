...
[32] T. S. Rappaport, *Wireless Communications: Principles and Practice*, 2nd ed. Upper Saddle River, NJ: Prentice Hall, 2002. ISBN 9780130422323.
— Verified via search (publisher listing, WorldCat, multiple bookseller records confirming 2nd ed./2002/Prentice Hall), 2026-08-31. Standard wireless-communications reference textbook, cited in Chapter 5 for the fundamental received-power/path-loss relationship used to cross-check the project's own diff-RT power formula.
...
"Gunnarsson et al. (2022), IEEE VTC, 'Prediction of urban radio propagation with 3D ray-tracing'" — cited as [GS22] in `results_differentiable_rt_calibration.md` on `claude/cool-cori-rrWbY` to justify preferring RT over COST-231 for irregular building height. A web search could not locate any paper matching this author/title/venue/year combination. Not used in Chapter 5's classical-model comparison; the same justification is instead grounded directly in COST 231's own stated model assumptions [10] (uniform building rows) versus Nottingham's documented building-height variation (Chapter 4).

"Degli-Esposti et al. (2004)" cited as [DeE04] in the same file for the claim that MSE-in-dBm is "the standard outdoor RT loss metric." Search results confirm Degli-Esposti's real, verified 2007 paper (already [27] in this list) but did not surface a distinct 2004 paper substantiating this specific methodological claim. Not cited in Chapter 5.

"The frequency (915 MHz) and environment ... are outside the COST 231 validity range" — also from `results_differentiable_rt_calibration.md`. COST 231 Walfisch–Ikegami's documented validity range is 800–2000 MHz, which includes 915 MHz; this claim is factually incorrect and is not repeated in Chapter 5. The genuine, verifiable reason ray tracing is preferred over COST-231 for this scenario is the building-height irregularity documented in Chapter 4 (COST-231 assumes uniform rooftops; Nottingham's urban core does not), not a frequency-range violation.

## Pending verification (not yet used in any chapter)
The specific ITU-R P.2040-1 material reflection-loss values [22] still need primary-source confirmation before final submission. Hardware/VM specs (GPU, CPU core count, RAM, VM/cluster type) are resolved and documented in Chapter 4, Section 4.2 — confirmed via a live `nvidia-smi` query on the training VM (hostname `sti-virtual-machine`).
